  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={loadData}
          className="self-start sm:self-auto flex items-center gap-2 px-3 py-2 text-xs font-semibold rounded-xl bg-slate-100 dark:bg-zinc-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors shadow-sm"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Total Views */}
        <div className="p-5 rounded-2xl bg-white dark:bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
          <div className="absolute right-3 top-3 w-10 h-10 rounded-xl bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-amber-600 dark:text-zinc-700 dark:text-zinc-300">
      ...
      <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-200 dark:border-zinc-800 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="font-bold text-lg text-slate-900 dark:text-white flex items-center gap-2">
              <span>Lưu lượng người đọc theo thời gian</span>
              {hoveredPoint && (
                <span className="text-xs font-normal text-amber-600 dark:text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center p-1 rounded-xl bg-slate-100 dark:bg-zinc-100 dark:bg-zinc-800 self-start sm:self-auto">
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {onNavigateTab && (
              <button
                ...
                className="text-xs font-semibold text-amber-600 dark:text-zinc-700 dark:text-zinc-300 hover:underline flex items-center gap-1"
              >
            ...
            {stats?.topStories.slice(0, 5).map((story, index) => {
              ...
              return (
                <div
                  ...
                  className="p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-100 dark:bg-zinc-800/60 transition-colors cursor-pointer border border-transparent hover:border-slate-200 dark:hover:border-zinc-300 dark:border-zinc-700"
                >
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black ${
                      index === 0
                        ? "bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-white shadow-sm"
                        : index === 1
                        ? "bg-slate-300 dark:bg-slate-700 text-slate-800 dark:text-white"
                        : index === 2
                        ? "bg-amber-700/20 text-amber-700 dark:text-amber-300"
                        : "bg-slate-100 dark:bg-zinc-100 dark:bg-zinc-800 text-slate-500"
                    }`}>
                      {index + 1}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Visual View Bar */}
                      <div className="w-full bg-slate-100 dark:bg-zinc-100 dark:bg-zinc-800 h-1.5 rounded-full overflow-hidden mt-2">
                        <div
                          className="bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 h-full rounded-full transition-all duration-500"
                          style={{ width: `${percent}%` }}
                        />
                      </div>
                    </div>
        ...
        <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2">
            ...
            {liveLogs.length === 0 ? (
              ...
              liveLogs.map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-100 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-7 h-7 rounded-lg bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-amber-600 dark:text-zinc-700 dark:text-zinc-300 flex-shrink-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-[11px] truncate">
                          Đã đọc: <span className="text-amber-600 dark:text-zinc-700 dark:text-zinc-300 font-medium">{log.chapterTitle}</span>
                        </p>
                      </div>
                    </div>
